...
InsertSolution::InsertSolution(TSPProblem *problem, vector<int> initial) : TSPSolution(problem, initial) {}
...
void InsertSolution::neighbourhood(NeighbourhoodParams p)
{
  ...
  for (int i = p.start; i < p.end - 1; i++)
  {
    for (int j = i + 1; j < p.problem->size(); j++)
    {
      ...
      // cost += p.problem->cost(p.path[j], p.path[i]);
      // // cost -= p.problem->cost(p.path[i], p.path[(i+1)%p.problem->size()]);
      // cost += p.problem->cost(p.path[(j - 1) % p.problem->size()], p.path[(j + 1) % p.problem->size()]);
