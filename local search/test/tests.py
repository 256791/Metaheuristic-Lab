...
def testMaxImpiter(instances, filename):
    ...
    r_pow2mullog2 = test(instances, max_imp_lmbd=lambda x: x['size']*x['size']*ceil(log(x['size'])))
...
def testTimePrd(instances, filename):
    data = test(instances)
    df = pd.DataFrame(columns=['instance_size', 'instance_name', 'insert_time',
                      'swap_time', 'invert_time', 'insert_prd', 'swap_prd', 'invert_prd'])
